...
namespace CXX
{
	...
	Object NativeFunction::newFunction(std::string name, Func callable, int arity, int optional)
	{
		return Object(std::make_shared<NativeFunction>(std::move(callable), std::move(name), arity, optional));
	}

	Object NativeFunction::newFunction(std::string name, std::function<void()> callable)
	{
		auto wrapper = [=](Interpreter&, const std::vector<Object>&) {
			callable();
			return Object();
		};
		return Object(std::make_shared<NativeFunction>(std::move(wrapper), std::move(name), 0, 0));
	}
